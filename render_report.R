...
#' Passes all options to Quality_Report.Rmd and renders HTML document
...
option_list <- list(
  ...
  make_option(c("-q", "--quantile_norm"), action="store_true", default=FALSE, type="logical",
              help="When flag is set, perform quantile normalization instead of using norm input file"),
...
tryCatch({
  rmarkdown::render(
    file.path(get_script_directory(), 'Probatch_Report_app_plots.Rmd'),
    ...
    params = list(
      ...
      iRT_annotation = args$iRT_annotation,
